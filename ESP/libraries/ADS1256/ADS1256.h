...
class ADS1256
{
	public:
		...
		//byte mux[ 8 ] = { 0x08,0x18,0x28,0x38,0x48,0x58,0x68,0x78 };    // Single Ended read of ADC channels
		byte mux[ 8 ] ={ 0x08,0x10,0x21,0x32,0x43,0x54,0x65,0x76 };   // Differential read of ADC channels
		
		ADS1256();

		void init( uint8_t _pinCS, uint8_t _pinRDY, uint8_t _pinRESET, uint32_t _speedSPI );
		void readInputToAdcValuesArray();
		void readInputToAdcValuesArrayMulti();
		void goStandby();
