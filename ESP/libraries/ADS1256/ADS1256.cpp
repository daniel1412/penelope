...
void ADS1256::init( uint8_t _pinCS, uint8_t _pinRDY, uint8_t _pinRESET, uint32_t _speedSPI )
{
	...
	***********************************************************************************************/
	...
	byte drate_data = 0x92; // F0h = 11110000 = 30,000SPS  -- 13h = 00010011 = 5SPS
...
void ADS1256::goStandby()
{
	SPI.beginTransaction( SPISettings( speedSPI, MSBFIRST, SPI_MODE1 ) ); // start SPI
	digitalWrite( pinCS, LOW );
	delayMicroseconds( 100 );

	//Begin Standby Mode (FDh)
	SPI.transfer( 0xFD );
	delayMicroseconds( 100 );
	digitalWrite(pinCS, HIGH);
	SPI.endTransaction();
	...
	***************************************************************************************/
	...
	{         // read all 8 Single Ended Channels AINx-AINCOM
		...
		*/
		...
		//WAKEUP 0000 0000
		SPI.transfer( 0x00 );
		delayMicroseconds( 201000 );   // Allow settling time
...
void ADS1256::readInputToAdcValuesArrayMulti()
{
	int i = 0;
	int j = 0;
	int meanCount = 1000;
	int values[ meanCount ];
	...
	for(i=0; i <= 7; i++){
		...
		for(j=0; j<=meanCount; j++){
			...
			values[j] = SPI.transfer( 0 );
			values[j] <<= 8; //shift to left
			values[j] |= SPI.transfer( 0 );
			values[j] <<= 8;
			values[j] |= SPI.transfer( 0 );
			delayMicroseconds( 2 );
		...
		adc_Raws[i] = valueMean/meanCount;
	}
