...
#else
  #define TIME_TO_SLEEP 295         // Time ESP32 will go to sleep (in seconds) - 6 Seconds for runtime  
#endif

// SQL Statements
char INSERT_SQL_T[] = "INSERT INTO Penelope.BME280_T_OUT(EnvSensorNr, Temperature, Humidity, Pressure, Altitude) VALUES(%d, %.2f, %.2f, %.2f, %.2f)";
char INSERT_SQL_V[] = "INSERT INTO Penelope.Batt_V_12V(EnvSensorNr, Voltage) VALUES(%d, %.2f)";
...
void getADCValue(){
  ...
  float maxV = 3.3;
  float corrFactor = 5.628; // Inverse of 1/6 -> used Voltage divider
  adcVoltage = maxV / 1024 * adcValue * corrFactor;
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println();
  Serial.println("Penelope Project: BME280 and ESP-ADC WiFi MySQL Esp8266 program");
  ...
  sprintf(sqlQuery, INSERT_SQL_T, 4, temperature, humidity, pressure, altitude);
  MySQLwrite();
  sprintf(sqlQuery, INSERT_SQL_V, 4, adcVoltage);
  MySQLwrite();
